...
export function defaultTenantProfile(tenantId: string): SandboxTenantProfile {
  return {
    tenantId,
    displayNameEn: tenantId === 'tnt_demo' ? 'Ceilgas Demo' : `Tenant ${tenantId}`,
    displayNameAr: tenantId === 'tnt_demo' ? 'تجربة سيلجاس' : `مستأجر ${tenantId}`,
  };
}
...
export function generateApiKeySecret(prefix = 'ceilgas_demo'): string {
